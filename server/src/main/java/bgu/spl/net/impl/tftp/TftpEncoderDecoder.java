...
public class TftpEncoderDecoder implements MessageEncoderDecoder<byte[]> {
    ...
    @Override
    //this is the server encode which is a bit different from the client's
    public byte[] encode(byte[] message) {
        opcode = (short) (((short) message[0]) << 8 | (short) (message[1]) & 0xff);
        //adding zero to the end of the message
        if(lastByteZero() && opcode != opBCAST){
